...
export class AuthService {
  ...
  ): Promise<User> 
  {
    ...
    if (user) {
      ...
      let existingUserByUsername = await this.usersService.findByUsername(username);
      let attempts = 0;

      while (existingUserByUsername && attempts < 10) { 
        username = `${profile.email.split('@')[0]}${Math.floor(Math.random() * 1000)}`;
        existingUserByUsername = await this.usersService.findByUsername(username);
        attempts++;
      }

      if (existingUserByUsername) {
          throw new ConflictException('Não foi possível gerar um nome de usuário único para login social.');
      }
